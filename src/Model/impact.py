...
def add_board_predictions(X,board_predict):
    ...
    ---
    Parameters:
        X (dataframe): Cleaned Data with Text, Source, Impact. 
...
def train_svm(X_train,Y_actual, save_model = "Y", model_name = "impact_train", export_path = "./Saved_Models/Impact", verbose=2):
    '''
    Train an SVM model using Board, BERT Encoded Text and Source. SVM model will be exported as 'impact_train.joblib'.
    ---
    Parameters:
        X_train (dataframe): BERT Encoded Text, one-hot-encoded (OHE) predicted Board and Source.
        Y_actual (list,array): True labels for Impact, between 0-2 (low-high).
...
def predict_svm(X_predict, import_path = "./Saved_Models/impact_train.joblib", verbose = 2):
    '''
